...
(function($, undefined) {
	...
	-----------------------------------------------------------------------------*/
        function updateEvent(event) {
            ...
            var i, len = cache.length, e, defaultEventEnd = getView().defaultEventEnd, // getView???
            startDelta = event.start - event._start, endDelta = event.end ? event.end - (event._end || defaultEventEnd(event)) : 0;
        ...
        function buildSkeleton(maxRowCnt, showNumbers) {
            ...
            for (i = 0; i < maxRowCnt; i++) {
                ...
                for (j = 0; j < colCnt; j++) {
                    ...
                    "<div>" + (showNumbers ? "<div class='fc-day-number'/>" : "") + "<div class='fc-day-content'>" + "<div style='position:relative'>&nbsp;</div>" + "</div>" + "</div>" + "</td>";
                }
                s += "</tr>";
            }
            s += "</tbody>" + "</table>";
            ...
            eventElement.resizable({
                ...
                resize: function(ev, ui) {
                    ...
                    if (slotDelta != prevSlotDelta) {
                        timeElement.text(formatDates(event.start, !slotDelta && !event.end ? null : // no change, so don't display time range
                        addMinutes(eventEnd(event), opt("slotMinutes") * slotDelta), opt("timeFormat")));
	...
	-----------------------------------------------------------------------------*/
        ...
        function daySegHTML(segs) {
            ...
            for (i = 0; i < segCnt; i++) {
                ...
                if (seg.isEnd && isEventResizable(event)) {
                    html += "<div class='ui-resizable-handle ui-resizable-" + (rtl ? "w" : "e") + "'>" + "&nbsp;&nbsp;&nbsp;" + // makes hit area a lot better for IE6/7
                    "</div>";
                }
                html += "</" + (url ? "a" : "div") + ">";
                seg.left = left;
