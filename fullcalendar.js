 ...
 *
 * Copyright (c) 2012 Adam Shaw
 * Dual licensed under the MIT and GPL licenses, located in
 * MIT-LICENSE.txt and GPL-LICENSE.txt respectively.
 *
 * Refactored and fixed by franklx (v1.5.5)
 *
 */
(function($, undefined) {
    ...
    function Header(calendar, options) {
        ...
        function renderSection(position) {
            ...
            if (buttonStr) {
                $.each(buttonStr.split(" "), function(i) {
                    ...
                    if (_this.indexOf("~") > -1) {
                        _this = _this.replace("~", "");
                        var g = $("<span class='btn-group' data-toggle='buttons-radio'></span>");
                    } else {
                        var g = $("<span class='btn-group'></span>");
                    }
                    e.append(g);
                    $.each(_this.split(","), function(j, buttonName) {
                        if (buttonName == "title") {
                            g.append("<span class='fc-header-title'><h2>&nbsp;</h2></span>");
                            if (prevButton) {
                            ...
                            if (buttonClick) {
                                ...
                                var button = $("<span class='btn fc-button-" + buttonName + "'>" + (icon ? "<span class='icon-" + icon + "'/>" : text) + "</span>");
                                if (button) {
        ...
        function activateButton(buttonName) {
            console.log("Active: " + buttonName);
            element.find("span.fc-button-" + buttonName).addClass("active");
        }
        function disableButton(buttonName) {
            element.find("span.fc-button-" + buttonName).addClass("disabled");
        }
        function enableButton(buttonName) {
            element.find("span.fc-button-" + buttonName).removeClass("disabled");
        }
        var t = this;
        t.render = render;
        t.destroy = destroy;
        ...
        function _fetchEventSource(source, callback) {
            ...
            if (events) {
                ...
                if (url) {
                    ...
                    if (startParam) {
                        data[startParam] = Math.round(+rangeStart / 1e3);
                    }
                    if (endParam) {
                        data[endParam] = Math.round(+rangeEnd / 1e3);
                    }
                    pushLoading();
    ...
    function parseDate(s, ignoreTimezone) {
        ...
        if (typeof s == "string") {
            if (s.match(/^\d+(\.\d+)?$/)) {
                return new Date(parseFloat(s) * 1e3);
            }
    ...
    function parseISO8601(s, ignoreTimezone) {
        ...
        if (ignoreTimezone || !m[13]) {
            ...
            if (m[12]) {
                date.setMilliseconds(Number("0." + m[12]) * 1e3);
            }
            fixDate(date, check);
        } else {
            date.setUTCFullYear(m[1], m[3] ? m[3] - 1 : 0, m[5] || 1);
            date.setUTCHours(m[7] || 0, m[8] || 0, m[10] || 0, m[12] ? Number("0." + m[12]) * 1e3 : 0);
            if (m[14]) {
                var offset = Number(m[16]) * 60 + (m[18] ? Number(m[18]) : 0);
                offset *= m[15] == "-" ? 1 : -1;
                date = new Date(+date + offset * 60 * 1e3);
            }
        }
        return date;
    }
    ...
    function AgendaEventRenderer() {
        ...
        function slotSegHtml(event, seg) {
            ...
            html += " class='" + classes.join(" ") + "'" + " style='position:absolute;z-index:8;top:" + seg.top + "px;left:" + seg.left + "px;" + skinCss + "'" + ">" + "<div class='fc-event-inner fc-event-skin'" + skinCssAttr + ">" + "<div class='fc-event-head fc-event-skin'" + skinCssAttr + ">" + "<div class='fc-event-time'>" + htmlEscape(formatDates(event.start, event.end, opt("timeFormat"))) + "</div>" + "</div>" + "<div class='fc-event-content'>" + "<div class='fc-event-title'>" + event.title + "</div>" + "</div>" + "<div class='fc-event-bg'></div>" + "</div>";
            if (seg.isEnd && isEventResizable(event)) {
    ...
    function DayEventRenderer() {
        ...
        function daySegHTML(segs) {
            ...
            for (i = 0; i < segCnt; i++) {
                ...
                html += "<span class='fc-event-title'>" + event.title + "</span>" + "</div>";
                if (seg.isEnd && isEventResizable(event)) {
    ...
    var dayIDs = [ "sun", "mon", "tue", "wed", "thu", "fri", "sat" ], DAY_MS = 864e5, HOUR_MS = 36e5, MINUTE_MS = 6e4;
    var dateFormatters = {
        s: function(d) {
            return d.getSeconds();
        },
